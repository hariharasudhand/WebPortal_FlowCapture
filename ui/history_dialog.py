...
class SessionHistoryDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.sort_combo.addItems([
            ...
            "Forms (most first)", "Forms (least first)",
            "Fields (most first)", "Fields (least first)"
        ])
        ...
        self.sessions_table.setHorizontalHeaderLabels([
            ...
            "Pages", "Forms", "Total Fields", "Named Fields", "Description"
        ])
        self.sessions_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeToContents)
        self.sessions_table.horizontalHeader().setSectionResizeMode(
            0, QHeaderView.Stretch)  # Website column stretches
        self.sessions_table.horizontalHeader().setSectionResizeMode(
            8, QHeaderView.Stretch)  # Description column stretches
        ...
        self.sessions_table.customContextMenuRequested.connect(
            self.showContextMenu)
        self.sessions_table.cellDoubleClicked.connect(
            self.onSessionDoubleClicked)
    ...
    def applyFilters(self):
        ...
        if search_text:
            all_sessions = [
                s for s in all_sessions if s.website and search_text in s.website.lower()]
        ...
        if self.use_date_filter.isChecked():
            ...
            from_timestamp = datetime.combine(
                from_date, datetime.min.time()).timestamp()
            to_timestamp = datetime.combine(
                to_date, datetime.max.time()).timestamp()

            all_sessions = [
                s for s in all_sessions if from_timestamp <= s.start_time <= to_timestamp]

        # Apply current sorting
        sort_index = self.sort_combo.currentIndex()
        if sort_index == 0:  # Date (newest first)
            all_sessions = sorted(
                all_sessions, key=lambda s: s.start_time, reverse=True)
        elif sort_index == 1:  # Date (oldest first)
            all_sessions = sorted(
                all_sessions, key=lambda s: s.start_time, reverse=False)
        elif sort_index == 2:  # Website (A-Z)
            all_sessions = sorted(
                all_sessions, key=lambda s: s.website.lower() if s.website else "")
        elif sort_index == 3:  # Website (Z-A)
            all_sessions = sorted(all_sessions, key=lambda s: s.website.lower(
            ) if s.website else "", reverse=True)
        elif sort_index == 4:  # Pages (most first)
            all_sessions = sorted(
                all_sessions, key=lambda s: s.page_count, reverse=True)
        elif sort_index == 5:  # Pages (least first)
            all_sessions = sorted(
                all_sessions, key=lambda s: s.page_count, reverse=False)
        elif sort_index == 6:  # Forms (most first)
            all_sessions = sorted(all_sessions, key=lambda s: getattr(
                s, 'forms_count', 0), reverse=True)
        elif sort_index == 7:  # Forms (least first)
            all_sessions = sorted(all_sessions, key=lambda s: getattr(
                s, 'forms_count', 0), reverse=False)
        elif sort_index == 8:  # Fields (most first)
            all_sessions = sorted(all_sessions, key=lambda s: getattr(
                s, 'total_fields', 0), reverse=True)
        elif sort_index == 9:  # Fields (least first)
            all_sessions = sorted(all_sessions, key=lambda s: getattr(
                s, 'total_fields', 0), reverse=False)
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(
                    self, "Success", "Session deleted successfully")
                self.loadSessions()
            else:
    ...
    def addDescriptionToSession(self):
        ...
        if ok:
            # Update session
            success = history_manager.update_session(
                session_id, description=new_desc)
    ...
    def exportSessions(self):
        ...
        if not self.filtered_sessions:
            QMessageBox.information(
                self, "No Data", "There are no sessions to export.")
            return
        ...
        try:
            ...
            QMessageBox.information(
                self, "Export Successful", f"Sessions exported to {file_path}")
        except Exception as e:
            QMessageBox.critical(self, "Export Error",
                                 f"Error exporting sessions: {str(e)}")
